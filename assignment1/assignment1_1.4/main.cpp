...
void separateArceus(arceus* &pArceus, string line)
{
	...
	while (iss)
	{
		...
		else if (sub.length() < MIN_CHAR_REQUIRE && sub.length() > 0 && sub != ";"){
			pArceus->piece_of_life = new piece;
			stringstream convert(sub);
			convert >> pArceus->piece_of_life->attribute;
			pArceus->piece_of_life->next = NULL;
		}
	}
}
